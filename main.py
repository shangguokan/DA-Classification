...
from dataset.loader import load_swda_corpus, load_mrda_corpus, tokenize_corpus
from sklearn.preprocessing import LabelBinarizer
from keras.preprocessing.sequence import pad_sequences

wv_dim = 64
vocab_size = 6000  # only applicable for "unigram" and "bpe" tokenization types
tokenization_type = 'bpe'

swda_conversation_list = swda_split.train_set_idx + swda_split.valid_set_idx + swda_split.test_set_idx
swda_corpus, swda_tag_set, swda_speaker_set, swda_user_defined_symbols = load_swda_corpus(
    swda_conversation_list,
    concatenate_interruption=True,
    do_lowercase=True,
    do_pretokenization=True if tokenization_type == 'word' else False,
    strip_punctuation=True
)

mrda_conversation_list = mrda_split.train_set_idx + mrda_split.valid_set_idx + mrda_split.test_set_idx
mrda_corpus, mrda_tag_set, mrda_speaker_set, mrda_user_defined_symbols = load_mrda_corpus(
...
swda_trainval_sentences = [sent for cid in swda_split.train_set_idx + swda_split.valid_set_idx for sent in swda_corpus[cid]['text']]
mrda_trainval_sentences = [sent for cid in mrda_split.train_set_idx + mrda_split.valid_set_idx for sent in mrda_corpus[cid]['text']]

utlis.train_and_save_tokenizer(
    swda_trainval_sentences + mrda_trainval_sentences,
    vocab_size=vocab_size,
    type=tokenization_type,
    user_defined_symbols='▁'+',▁'.join(list(swda_user_defined_symbols)),
...
swda_corpus = tokenize_corpus(swda_corpus, tokenizer)
mrda_corpus = tokenize_corpus(mrda_corpus, tokenizer)

swda_trainval_sentences = [sent for cid in swda_split.train_set_idx + swda_split.valid_set_idx for sent in swda_corpus[cid]['tokenized_text']]
mrda_trainval_sentences = [sent for cid in mrda_split.train_set_idx + mrda_split.valid_set_idx for sent in mrda_corpus[cid]['tokenized_text']]

utlis.train_and_save_word2vec(swda_trainval_sentences + mrda_trainval_sentences, wv_dim=wv_dim, epochs=30, path='resource/wv_swda.bin')
